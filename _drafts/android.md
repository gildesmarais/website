...
i create graphical user interfaces every day. i can not only use ios to create UIs to be used on any mobile device. the resulting knowledge wouldn't be diverse. i personally don't want to deliberately build a GUI which could be equivalent to a [soap dispenser that only dispenses to white hands](https://invidio.us/watch?v=WHynGQ9Vg30).

PWA:

bei android sind progressive web apps besser als unter ios supported. unter ios das noch gar nich möglich. bei einem testprojekt habe ich festgestellt, dass das logo viel klein angezeigt wird. ich hätte es ohne testen auf dem droiden nie gemerkt :)

## The device

I'm quite surprised by the Motorola G6 Plus. It's a 2018 model. It costs about 170€ now. You'll get a lot of phone. The battery lasts (for me as iOS user) unusually long (about 5 days with little use).

I have chosen this device, because:

1. its bootloader is unlockable
2. LineageOS supports the device <q>officially</q>
3. i could buy an unused "shipping return" for even less.

Before you can install LineageOS you're advised to update to the latest available security patch provided by the manufacturer. [Pal did the tiresome work.]()
TODO: post link
...
This feeling of uncertainty when playing with hardware and flashing things is exciting and unique.
...
I've never dealt with Android before. Many concepts and implementations differ from ios and are not <q>wrong</q>. Some things I miss in iOS (setting standard apps...) are provided by the system and their customization is well integrated.

Cal- and CardDAV is missing out of the box. Also, there is no non-google email client installed. All three essential can be installed and the system can be upgraded accordingly. FairMail and davX5 do that for me. I bought a license for FairMail.

What Google tries to press on the phone during the installation is questionable: "we're going to install our office apps" - a cancellation is not possible, pressing OK the only possible action.
in retrospect that's no surprise: Google wants to make the use of its services as easy as possible and installs them right away. one could also formulate this behaviour less benevolently...
...
The android keyboard provides a great typing experience and is superior to any iOS keyboard available. I mistype much less than on an iOS keyboard. By default the keyboard gives a haptic feedback when pressing a key. I couldn't stand that long and had to turn it off.
...
Android comes with alarm clock sounds which truly reassemble an alarm clock... some things are easy to get right (but are apparently hard to implement). Compare that to iOS' Clock app, which is UX nightmare.
...
Of course you can use DNS over HTTPS natively and chose a DNS server which blocks ads, e.g. [dnsforge](https://dnsforge.de/).

### Exceptional iOS apps

Some people told me one reason they'd never switch to Android is that some apps on iOS are exceptional and no counter-parts on Android would exist. That's true[^2].
...
As counter-example I'd mention the Klack app. The Android version is more polished than its iOS counterpart.

This leads me to a conclusion: if you need an app for your product, it is probably wise to utilize Flutter or React Native to get you up and running. If you can't do that (or decide against it), be very strict with features which should be available on any platform and sync their release schedules.
...
I installed [microG](https://microg.org/) (a [prototypefund funded project](https://prototypefund.de/project/microg/)), but nonetheless I try to cut as much connections to Google as possible. The term for that is _degoogle_. Several guides to _degoogle_ exist, although some aren't updated for Lineage 17.1 yet:

- [How to deGoogle LineageOS in 2019](https://www.reddit.com/r/degoogle/comments/cldohl/how_to_degoogle_lineageos_in_2019/)

If you're interested in not using Google service in general, skim through the awesome list called <q>[Cutting Google out of your life](https://degoogle.jmoore.dev/)</q>.
