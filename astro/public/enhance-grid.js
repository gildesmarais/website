  ...
  for (const grid of grids) {
    ...
    const applyFilters = (updateHistory = true) => {
      ...
      items.forEach((item) => {
        ...
        const filterMatch = [...currentFilters.entries()].every(([key, value]) => item.dataset[key] === value)
      ...
      if (updateHistory) {
        ...
        const sortButton = document.querySelector("[data-sort-by][data-sort-dir]")
        if (sortButton) {
    ...
    const toCamelCase = (s) =>
      s.replace(/([-_][a-z])/g, (g) => g.toUpperCase().replace("-", "").replace("_", ""))

    params.forEach((value, key) => {
      if (key !== "q" && key !== "sort" && key !== "dir") {
        // Handle the parameter name as-is first, then try camelCase conversion
        let targetKey = key
        let btn = document.querySelector(`[data-filter-by="${targetKey}"][data-filter-value="${value}"]`)
        ...
        if (!btn && (key.includes("-") || key.includes("_"))) {
          ...
          btn = document.querySelector(`[data-filter-by="${targetKey}"][data-filter-value="${value}"]`)
        }
